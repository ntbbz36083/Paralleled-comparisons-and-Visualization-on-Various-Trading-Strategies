...
for (i in seq(1,ncol(stock.AllData)/6*3,3))   #### seq(1,n*3=1428,3)
{
  ...
  for (day in 11:nrow(Tech_RVI))
  {
    ...
    if(Tech_RVI[day,i+1] > Tech_RVI[day,i+2] && Tech_RVI[day-1,i+1] < Tech_RVI[day-1,i+2] && a%%2==1 | (News_signal[day,(i+2)*2+1] == 'ENTRY' && a%%2==1))
    {
    ...
    if(a%%2==0)
    {
      ...
      if(Tech_RVI[day,i+1] < Tech_RVI[day,i+2] && Tech_RVI[day-1,i+1] > Tech_RVI[day-1,i+2] | News_signal[day,(i+2)*2+1] == 'EXIT')
      {
        trade_2[a,b] <- -Tech_RVI[day,i]
        VWMA.4[a,(i+2)/3] <- Tech_RVI[day,i+2]### for plot
        HoldDay <- day-BuyDay
  ...
  if(trade_2[a-1,b] > 0 && trade_2[a,b] == 0 && a%%2 == 0)
  {
